...
import (
	...
	//"mediaserver-go/thirdparty/thirdparty/avutil"
)
...
func (s *Stream) SetCodecParamForMuxer(codecID avcodec.CodecID, mediaType avutil.MediaType, sampleRate int) {
	codecpar := s.CodecParameters()
	codecpar.SetCodecID(codecID)
	codecpar.SetCodecType(mediaType)
	codecpar.SetSampleRate(sampleRate)
	if mediaType == avutil.AVMEDIA_TYPE_VIDEO {
		codecpar.SetWidth(1280)
		codecpar.SetHeight(720)
		codecpar.SetFormat(8)
	} else {
		codecpar.AvChannelLayoutDefault(2)
	}
}
...
func (s *Stream) SetTimeBase(num, den int) {
	s.time_base.num = C.int(num)
	s.time_base.den = C.int(den)
}
