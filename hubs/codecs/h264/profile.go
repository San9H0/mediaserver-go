...
// reference from https://github.com/jiyeyuran/mediasoup-go/blob/master/h264/h264profile.go
...
func (r RtpParameter) String() string {
	if r.PacketizationMode != nil {
		return fmt.Sprintf("level-asymmetry-allowed=%d;packetization-mode=%d;profile-level-id=%s",
			r.LevelAsymmetryAllowed,
			*r.PacketizationMode,
			r.ProfileLevelId)
	}
	return fmt.Sprintf("level-asymmetry-allowed=%d;profile-level-id=%s",
		r.LevelAsymmetryAllowed,
		r.ProfileLevelId)
}
