...
type AV1Parser struct {
	config *codecs.AV1Config

	av1Packet pioncodec.AV1Packet
	av1Frame  frame.AV1
...
func NewAV1Parser(config *codecs.AV1Config) *AV1Parser {
	return &AV1Parser{
		config: config,
	}
}
		...
		if index == 0 && Z == 1 { // 이전요소가 이어짐.
			...
			if N == 1 && index == 0 {
				...
				if err := header.Unmarshal(data); err == nil {
					av1Codec, _ := codecs.NewAV1(header.Width(), header.Height())
